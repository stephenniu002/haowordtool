export async function adspowerStatus(env,fetcher=fetch){
  ...
  const url=new URL(env.ADSPOWER_API_BASE||'http://127.0.0.1:50325');
  ...
  try{
    ...
    return {configured:true,active:response.ok&&result.code===0&&result.data?.status==='Active',reason:result.code===0?null:'AdsPower API authentication or profile check failed',publishingVerified:false};
