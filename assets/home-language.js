(() => {
  ...
  let request=0, current='en', cached;
  const read=(data,key)=>key.split('.').reduce((obj,k)=>obj?.[k],data);
  async function setLanguage(lang, updateURL=true){
    if(!['en','zh-CN'].includes(lang)) lang='en';
    const id=++request;
    try {
      if(lang==='zh-CN'&&!cached){const r=await fetch('/locales/zh-CN.json?v=home-language-1');if(!r.ok)throw Error('Language unavailable');const data=await r.json();if(bindings.some(b=>typeof read(data,b.key)!=='string'))throw Error('Incomplete language');cached=data;}
      if(id!==request)return;
      for(const b of bindings){const value=lang==='en'?b.original:read(cached,b.key);if(b.attr)b.el.setAttribute(b.attr,value);else b.el.textContent=value;}
      current=lang;select.value=lang;document.documentElement.lang=lang;
      document.title=lang==='zh-CN'?'HaoWordTool — 更好的商品，更聪明的 AI 工具':originalTitle;
      ...
      status.textContent=lang==='zh-CN'?'首页已切换为中文':'Homepage switched to English';
      document.dispatchEvent(new CustomEvent('home-language-change',{detail:{language:lang}}));
    }catch{if(id!==request)return;select.value=current;status.textContent='中文暂时无法加载，请稍后重试。 Language could not be loaded. Please try again.';}
  }
