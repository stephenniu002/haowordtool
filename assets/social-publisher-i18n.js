(() => {
  ...
  rows.push(
    ...
    ['Customers provide their own platform accounts and complete sign-in. Publishing times are configured with the service provider. Do not send passwords or login cookies through this form.','客户提供自己的各平台账号并完成登录，发布时间由服务方协助设置。请勿通过本表单发送密码或登录 Cookie。','お客様ご自身の各プラットフォームのアカウントでログインしてください。投稿日時はサービス提供者と設定します。このフォームでパスワードやログインCookieを送らないでください。','Les clients utilisent leurs propres comptes et effectuent la connexion. Les horaires sont configurés avec le prestataire. N’envoyez ni mots de passe ni cookies de connexion via ce formulaire.','Los clientes aportan sus propias cuentas e inician sesión. Los horarios se configuran con el proveedor del servicio. No envíes contraseñas ni cookies de sesión por este formulario.'],
    ['Publishing preference','发布方式','投稿方法','Mode de publication','Modo de publicación'],
    ['Publish as soon as ready','立即发布（准备就绪后）','準備でき次第投稿','Publier dès que prêt','Publicar cuando esté listo'],
    ['Scheduled publishing','定时发布','予約投稿','Publication programmée','Publicación programada'],
    ['Requested date and time','期望发布日期和时间','希望する投稿日時','Date et heure souhaitées','Fecha y hora deseadas'],
    ['Time zone (UTC offset)','时区（UTC 偏移）','タイムゾーン（UTCオフセット）','Fuseau horaire (décalage UTC)','Zona horaria (desfase UTC)'],
    ['Choose the UTC offset for the publishing date; daylight saving time may change the offset.','请选择发布日期对应的 UTC 偏移；夏令时可能改变时差。','投稿日に適用されるUTCオフセットを選んでください。夏時間により変わる場合があります。','Choisissez le décalage UTC à la date de publication ; l’heure d’été peut le modifier.','Elige el desfase UTC de la fecha de publicación; el horario de verano puede modificarlo.'],
    ['This is a service request, not an active publishing job. Login and platform support must be confirmed.','这里只记录服务需求，尚未创建实际发布任务。需确认账号登录及平台支持情况。','これはサービス依頼です。投稿予約はまだ作成されていません。ログインとプラットフォームの対応確認が必要です。','Ceci est une demande de service, pas une publication active. La connexion et la compatibilité doivent être confirmées.','Esto es una solicitud de servicio, no una publicación activa. Hay que confirmar la sesión y la compatibilidad.'],
    ['Scheduling is listed upstream for Douyin, TikTok, Xiaohongshu, Bilibili, Kuaishou and WeChat Channels. The current YouTube uploader does not support scheduling.','原项目列出国内抖音、TikTok、小红书、B站、快手和视频号支持定时发布；当前 YouTube 上传模块不支持定时。','元プロジェクトではDouyin、TikTok、小紅書、Bilibili、Kuaishou、WeChat Channelsが予約投稿に対応しています。現在のYouTubeアップローダーは非対応です。','Le projet d’origine indique la programmation pour Douyin, TikTok, Xiaohongshu, Bilibili, Kuaishou et WeChat Channels. Le module YouTube actuel ne la prend pas en charge.','El proyecto original indica programación para Douyin, TikTok, Xiaohongshu, Bilibili, Kuaishou y WeChat Channels. El módulo actual de YouTube no la admite.'],
    ['Choose a future date and time in the selected UTC offset.','请选择在所选时区尚未到来的日期和时间。','選択したUTCオフセットで未来の日時を指定してください。','Choisissez une date et une heure futures dans le décalage UTC sélectionné.','Elige una fecha y hora futuras en el desfase UTC seleccionado.']
  );
