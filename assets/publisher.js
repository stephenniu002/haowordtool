...
function updateShowcase(platforms=[]){const hasState=platforms.length>0;for(const card of document.querySelectorAll('#platformShowcase [data-platform]')){const label=card.querySelector('b')?.textContent||card.dataset.platform,p=platforms.find(item=>platformMatches(item,card.dataset.platform,label)),status=card.querySelector('small');if(hasState){card.classList.toggle('active',!!p?.available);if(status)status.textContent=p?.available?'点击连接':p?(card.dataset.auth||'待配置'):'未接入';}card.onclick=e=>{e.preventDefault();directConnect(card.dataset.platform,label).catch(err=>notice(err.message));};}}
