...
function apply(){const b=data();$('#chat-name').textContent=b.name;$('#avatar').dataset.portrait=String(selected);$('#avatar').dataset.maturePortrait=profiles[selected].maturePortrait??'';$('#avatar').textContent=b.name.slice(0,1);updateAvatar();$('#messages').replaceChildren();bubble(greetings[b.language]);}
